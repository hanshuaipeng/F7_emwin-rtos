...
//更新时的提示信息
uint8_t*const UPDATE_REMIND_TBL[5]=
{
...
uint32_t fupd_prog(uint16_t x,uint16_t y,uint8_t size,uint32_t fsize,uint32_t pos)
{
	...
	if(t!=prog)
	{
		LTDC_ShowString(x+3*size/2,y,size,"%");		
		t=prog;
		if(t>100)t=100;
		LTDC_ShowNum(x,y,t,3,size);//显示数值
	}
	return 0;					    
} 
//更新某一个
...
{	
	...
	if(rval==0)				//字库文件都存在.
	{  
		LTDC_ShowString(x,y,size,"Erasing sectors... ");//提示正在擦除扇区	
		...
		{
			LTDC_ShowString(x,y,size,UPDATE_REMIND_TBL[i]);		
			strcpy((char*)pname,(char*)src);				//copy src内容到pname
			strcat((char*)pname,(char*)GBK_PATH[i]); 		//追加具体文件路径 
