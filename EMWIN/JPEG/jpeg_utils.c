				...
				for(k=0;k<2;k++)
				{
					...
					{     
						...
						*((vuint32_t*)(pOutAddr2+4))=
						...
						ycomp=(int)(*(pLum+j));   
						*(__IO uint16_t*)pOutAddr=CLAMP(ycomp+c_red)|CLAMP(ycomp+c_green)|CLAMP(ycomp+c_blue);
						ycomp=(int)(*(pLum+j+1)); 
						*((__IO uint16_t*)pOutAddr+1)=CLAMP(ycomp+c_red)|CLAMP(ycomp+c_green)|CLAMP(ycomp+c_blue);
						ycomp=(int)(*(pLum+j+8)); 
						*(__IO uint16_t*)pOutAddr2=CLAMP(ycomp+c_red)|CLAMP(ycomp+c_green)|CLAMP(ycomp+c_blue);
 						ycomp=(int)(*(pLum+j+8+1));   
						*((__IO uint16_t*)pOutAddr2+1)=CLAMP(ycomp+c_red)|CLAMP(ycomp+c_green)|CLAMP(ycomp+c_blue); 
#endif  
...
{  
	static __IO uint16_t tempadd=0;
	uint32_t numberMCU;
	uint32_t i,j,k,currentMCU,xRef,yRef;
	uint32_t refline; 
	int ycomp,crcomp,cbcomp;
...
#if JPEG_BYTES_PER_PIXEL==4		//ARGB_8888颜色格式    
					*(vuint32_t*)pOutAddr=ySample|(ySample<<8)|(ySample<<16); 
#elif JPEG_BYTES_PER_PIXEL==3	//RGB_888颜色格式  
					pOutAddr[0]=ySample;
					pOutAddr[1]=ySample;
					pOutAddr[2]=ySample; 
#elif JPEG_BYTES_PER_PIXEL==2	//RGB_565颜色格式
					*(__IO uint16_t*)pOutAddr=(ySample&0XF8)<<8|(ySample&0XFC)<<3|ySample>>3;
#endif   
					pOutAddr+=JPEG_BYTES_PER_PIXEL;
					pLum++;
				}
...
#elif JPEG_BYTES_PER_PIXEL==2	//RGB_565颜色格式 
					*(__IO uint16_t*)pOutAddr=(c_red&0XF8)<<8|(c_green&0XFC)<<3|c_blue>>3;
#endif      
					pOutAddr+=JPEG_BYTES_PER_PIXEL; 
					pChrom++;
				} 
