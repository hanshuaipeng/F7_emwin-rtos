...
//返回值:0,成功
//    其他,失败
uint8_t hjpgd_decode(uint8_t* pname)
{
	FIL* ftemp; 
	u16* rgb565buf;
	vuint32_t timecnt=0;
	...
	JPEG_IN_OUT_DMA_Init((uint32_t)hjpgd.inbuf[0].buf,(uint32_t)hjpgd.outbuf[0].buf,hjpgd.inbuf[0].size,JPEG_DMA_OUTBUF_LEN);//配置DMA
	...
	while(1)
	{
		...
		{
			mcublkindex+=hjpgd.ycbcr2rgb(hjpgd.outbuf[hjpgd.outbuf_read_ptr].buf,(uint8_t*)rgb565buf,mcublkindex,hjpgd.outbuf[hjpgd.outbuf_read_ptr].size); 
