...
#define XSIZE_PHYS 480     //最大支持1024*768
...
#define LCD_LAYER1_FRAME_BUFFER  ((uint32_t)0xC01f4000)     //第二层缓冲区
...
void LCD_X_Config(void) 
{
    ...
    if(lcdltdc.dir==0)//竖屏
    {
        LCD_SetSizeEx(0,lcdltdc.width,lcdltdc.height);    //设置可见区尺寸
        LCD_SetVSizeEx(0,lcdltdc.width,lcdltdc.height*NUM_VSCREENS);   //设置虚拟显示区尺寸
    ...
    GUI_TOUCH_Calibrate(GUI_COORD_X,0,lcdltdc.width,0,lcdltdc.width-1);   
    GUI_TOUCH_Calibrate(GUI_COORD_Y,0,lcdltdc.height,0,lcdltdc.height-1);
#if (GUI_NUM_LAYERS>1)
    ...
    if(lcdltdc.dir==0)//竖屏
    {
        LCD_SetSizeEx(1,lcdltdc.width,lcdltdc.height);    //设置可见区尺寸
        LCD_SetVSizeEx(1,lcdltdc.width,lcdltdc.height*NUM_VSCREENS);   //设置虚拟显示区尺寸
        GUI_SetOrientation(GUI_SWAP_XY|GUI_MIRROR_Y);                   //设置为竖屏
    }else            //横屏
    {
    	LCD_SetSizeEx(1,lcdltdc.width,lcdltdc.height);    //设置可见区尺寸
...
static void LCD_LL_LayerInit(uint32_t LayerIndex) 
{
    ...
    if (LayerIndex<GUI_NUM_LAYERS) 
    {
        ...
        HAL_LTDC_ConfigLayer(&hltdc,&layer_cfg,LayerIndex);//设置所选中的层  
