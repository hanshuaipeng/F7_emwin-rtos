...
extern uint8_t* const GBK_PATH[5];
extern uint8_t* const UPDATE_REMIND_TBL[5];  
...
//fsize:整个文件大小
//pos:当前文件指针位置
uint32_t emwin_fupd_prog(uint32_t fsize,uint32_t pos)
{
    WM_HWIN hItem;
	float prog;
    char buffer[10];
...
//返回值:0,成功;其他,失败.
uint8_t emwin_updata_fontx(uint8_t *fxpath,uint8_t fx)
{
...
//返回值:0,更新成功;
//		 其他,错误代码.	  
uint8_t emwin_update_font(uint8_t* src)
{	
    WM_HWIN hItem;
	uint8_t *pname;
	uint32_t *buf;
