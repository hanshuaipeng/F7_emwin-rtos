...
//	taskEXIT_CRITICAL();            //退出临界区 
	...
	switch(mode)
	{
		case 0:	//在指定位置显示图片
			...
			break;
		case 1:	//在LCD中间显示图片
			GUI_PNG_Draw(pngbuffer,PNGFile.obj.objsize,(lcdltdc.width-XSize)/2-1,(lcdltdc.height-YSize)/2-1);	
			break;
	}
...
int displaypngex(char *PNGFileName,u8 mode,u32 x,u32 y)
{
	...
	switch(mode)
	{
		case 0:	//在指定位置显示图片
			...
			break;
		case 1:	//在LCD中间显示图片
			GUI_PNG_DrawEx(PngGetData,&PNGFile,(lcdltdc.width-XSize)/2-1,(lcdltdc.height-YSize)/2-1);
			break;
	}
	f_close(&PNGFile);	//关闭PNGFile文件
	return 0;
