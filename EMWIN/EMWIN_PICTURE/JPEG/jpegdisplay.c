...
int displyjpeg(u8 *JPEGFileName,u8 mode,u32 x,u32 y,int member,int denom)
{
	...
	switch(mode)
	{
		...
		case 1:	//在LCD中间显示图片
			...
			{
				//在LCD中间显示图片
				GUI_JPEG_Draw(jpegbuffer,JPEGFile.obj.objsize,(lcdltdc.width-XSize)/2-1,(lcdltdc.height-YSize)/2-1);
			}else //否则图片需要缩放
			{
...
int displayjpegex(u8 *JPEGFileName,u8 mode,u32 x,u32 y,int member,int denom)
{
	...
	switch(mode)
	{
		...
		case 1:	//在LCD中间显示图片
			...
			{
				//在LCD中间显示图片
				GUI_JPEG_DrawEx(JpegGetData,&JPEGFile,(lcdltdc.width-XSize)/2-1,(lcdltdc.height-YSize)/2-1);
			}else //否则图片需要缩放
			{
				Xflag = (float)XSize*((float)member/(float)denom);
				Yflag = (float)YSize*((float)member/(float)denom);
