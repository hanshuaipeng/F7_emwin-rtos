	...
	switch(mode)
	{
		...
		case 1:	//在LCD中间显示图片
			if((member == 1) && (denom == 1)) //无需缩放，直接绘制
			{
				...
				for(i=0;i<GifInfo.NumImages;i++)
				{
					GUI_GIF_DrawSub(gifbuffer,GIFFile.obj.objsize,(lcdltdc.width-XSize)/2-1,(lcdltdc.height-YSize)/2-1,i);
					GUI_GIF_GetImageInfo(gifbuffer,GIFFile.obj.objsize,&ImageInfo,i);
					GUI_Delay(ImageInfo.Delay ? ImageInfo.Delay*10:100);//延时
				}
			}else //否则图片需要缩放
	...
	switch(mode)
	{
		...
		case 1:	//在LCD中间显示图片
			if((member == 1) && (denom == 1)) //无需缩放，直接绘制
			{
				...
				for(i=0;i<GifInfo.NumImages;i++)
				{
					GUI_GIF_DrawSubEx(GifGetData,&GIFFile,(lcdltdc.width-XSize)/2-1,(lcdltdc.height-YSize)/2-1,i);
					GUI_GIF_GetImageInfoEx(GifGetData,&GIFFile,&ImageInfo,i);
					GUI_Delay(ImageInfo.Delay ? ImageInfo.Delay*10:100);//延时
				}
			}else //否则图片需要缩放
